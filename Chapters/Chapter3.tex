...
$z$. $\phi_{s_{1},\dots,s_{n}}^{\alpha_{1},\dots,\alpha_{n}}(\mathbf{T}_{1},\dots,\mathbf{T}_{n})$ are the $n$ body force-constants ($n$BFC). For the 2BFC we omit the index $(2)$. \\
...
\begin{equation}
 \label{2bfc-fourier}
 \overset{(n)}{\phi}{}_{s_{1}\dots s_{n}}^{\alpha_{1}\dots\alpha_{n}}(\mathbf{T}_{1},\dots,\mathbf{T}_{n})=\frac{1}{N_{\mathbf{q}}^{n}}\sum_{\mathbf{q}_{1}\dots\mathbf{q}_{n}}e^{i(\mathbf{q}_{1}\cdot\mathbf{T}_{1}+\dots+\mathbf{q}_{
...
We take advantage of the lattice periodicity also in this case and, Fourier transforming with respect to the lattice indices, we consider separated spectral functions $-2ImTr[\boldsymbol{G}(\boldsymbol{q},\omega+i0^{+})]$ for 
...
\begin{equation}
\label{lifetime}
 \tau_{\mu}(\mathbf{q})=\frac{1}{\Gamma_{\mu}(\mathbf{q})}.
\end{equation}
...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [block, below of=precision, node distance=3cm] (output) {Output quantities $\mathcal{H}=\mathcal{H}_{j}$, $\mathcal{F}_{H^{I}}[\mathcal{H}]$, $\boldsymbol{\mathcal{R}}_{\mathcal{H}}$, $\tilde{\Omega}_{\mu\mathcal{H}}$, and $\boldsymbol{\tilde{\epsilon}}_{s\mu\mathcal{H}}$};
    % Draw edges
