...
is the full width at half maximum divided by a factor of $1.5$). The calculations are done within LDA in the experimental structure using SSCHA 3BFC at $800$ K and $\tilde{\Omega}_{\mu}(\mathbf{q})$ at $800$ K.}
...
performed within LDA in the experimental structure. In each case we use $\tilde{\Omega}_{\mu}(\mathbf{q})$ calculated at the same temperature as the SSCHA 3BFC.}
\label{spectral-snse}
\end{center}
\end{figure}
