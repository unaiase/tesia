...
transition\cite{zhao2014ultralow,adouby1998structure,chattopadhyay1986neutron,von1981high} at around $800$ K from the low-symmetry $Pnma$ phase to the high-symmetry $Cmcm$. See Fig. \ref{pnma-cmcm} for the crystal 
...
adouby1998structure,chattopadhyay1986neutron} claim it is a second-order phase transition and others\cite{von1981high} it has a first-order character. A recent work\cite{dewandre2016two} argues that the transition occurs in 
two steps, where increasing temperature induces first a change in the lattice parameters that induces after a lattice instability. There is no inelastic scattering experiment so far for the high-temperature phase, which should 
show a prominent phonon collapse at the transition temperature if it belonged to the displacive second-order type\cite{holt2001x,weber2011extended,o2017inelastic}. \\
...
The group/subgroup index of the $Cmcm$/$Pnma$ transition is $2$, making a displacive second-order transition posible\cite{toledano1987landau}. In this scenario, the transition temperature $T_{c}$ is defined as the temperature 
...
the $Pnma$ and $Cmcm$. We check that a SCHA calculation at $T>T_{c}$ ($T=800$ K) starting from the relaxed low-symmetry $Pnma$ phase (relaxed at DFT static level) yields the high-symmetry $Cmcm$ atomix positions for the 
$\boldsymbol{\mathcal{R}}$ centroids. Thus, the $Pnma$ is not a local minimum of the free energy above $T_{c}$, ruling out the first-order transition. Our result disagrees with the conclusions drawn in Ref. \cite{dewandre2016two}.
...
is consistent with the condensation of the $Y_{1}$ mode after the transition. \\
...
is the full width at half maximum). The calculations are done within LDA in the experimental structure using SCHA 3BFC at $800$ K and $\tilde{\Omega}_{\mu}(\mathbf{q})$ at $800$ K.}
...
($R_{1}$) at $R$. The instabilities at $\Gamma$ would cause ferroelectric transitions\cite{skelton2016anharmonicity,hong2016electronic}, but they suffer an anharmonic renormalization that prevents it. $Y_{3}$ and $R_{1}$ are also 
stabilized by anharmonic effects. The $Y_{1}$ mode however remains unstable at $600$ K and it is stabilized after the transition (see Fig. \ref{spectral-snse}). \\

In highly anharmonic materials\cite{ribeiro2018strong,li2014phonon,bianco2018high,delaire2011giant,paulatto2015first}, the spectral functions show broad peak, shoulders and satellite peaks, strongly deviating from the Lorentzian 
