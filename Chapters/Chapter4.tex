...
As we will deal with highly pure solids at temperatures that are higher than the Debye temperature, we will only 
...
\begin{multline}
 ...
 \delta(\omega_{\mu}(\mathbf{q})-\omega_{\mu_{1}}(\mathbf{q}_{1})-\omega_{\mu_{2}}(\mathbf{q}_{2})).
\end{multline}
In the previous equations we have written the anharmonic 3BFC in normal coordinates space. For that purpose we need to apply the following transformation to the atomic displacements in the nBFC 
...
positive semidefinite\cite{fugallo2013ab} and it can be splitted as $\mathbf{A}=\mathbf{A}^{in}+\mathbf{A}^{out}$ where
\begin{equation}
